...
export const onRequestGet = async ({ env, request }) => {
  ...
  const [blogCount, progCount, leadCount, feedbackRows, viewRows, aiCost, latestLeadsRows] = await Promise.all([
    ...
    env.DB.prepare(`SELECT id, name, email, phone, source, blog_slug, created_at FROM leads ORDER BY created_at DESC LIMIT 5`).all().catch(() => ({ results: [] })),
  ]);
  ...
  return json(200, {
    ...
    latest_leads: latestLeadsRows.results || [],
  });
};
