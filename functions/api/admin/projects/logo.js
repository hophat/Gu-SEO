// POST /api/admin/projects/logo
//   Body: { filename, content_type, base64 }
//
// Uploads a logo to R2 and points projects.logo_url at it. Scoped to the
// caller's active project, so a project admin can only replace their own
// logo. Mirrors the cover-asset upload contract (JSON base64 rather than
// multipart) so the admin UI can reuse one file-reading helper.
...
export const onRequestPost = async ({ env, request }) => {
  ...
  const key = `project/${tenant.activeProjectId}/logo/${newId()}.${extFor(mime)}`;
  try {
